...
export interface AutocompleteCallbackEntry {
  fn: (index: number, tokens: string[], ...args: string[]) => string | string[],
  args: string[],
}
 ...
 */
export function collectAutocompleteCandidates(callbacks: AutocompleteCallbackEntry[], input: string) {
  const tokens = parse(input) as string[]
